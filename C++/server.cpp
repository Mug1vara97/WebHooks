...
string show_all_buscket(json request_parse){
    string user_id = request_parse["session"]["user"]["user_id"];
    fstream fs(user_id + ".txt", std::ios::app | std::ios::out | std::ios::in);
    string line;
    string buscket;
...
void delete_from_buscket(json request_parse, string user_message){
    ...
    std::fstream fs(user_id + ".txt", std::ios::app | std::ios::out);
    fs << all_buscket << endl;
}
...
int main() {
    ...
    svr.Post("/", [](const httplib::Request& req, httplib::Response& res) {
        ...
        json request_parse = parse_req(req,res);
        json response_parse_button_ability = parse_res("ability");
        json response_parse_to_main_page = parse_res("to_main_page");
...
//g++ server.cpp -o test --std=c++14 -I/usr/local/include -I/usr/include/nlohmann httplib.h
//если cmake не работает то запускать через эту строчку в консоли